function getRandomNumber(min, max,last) {
    min = Math.ceil(min);
    max = Math.floor(max);
    rez=Math.floor(Math.random() * (max - min)) + min;
    if(rez!=last){return rez} 
    else{return getRandomNumber(min,max,last)}
}
let flagGirlianda=false;
let girlanda=document.querySelector(".garland-new-year-part")
let delay = 500;
let numberOfCard=1;
let miganieGirliandy=setInterval(function() {
    if(flagGirlianda){
    numberOfCard=getRandomNumber(2,6,numberOfCard);
    let adressOfCard="girlanda"+numberOfCard+".jpg";
    girlanda.src=adressOfCard;}
    }, delay);

girlanda.addEventListener("click",()=>{
    if(flagGirlianda==false){flagGirlianda=true;
        
    }
    else{flagGirlianda=false;
        girlanda.src="girlanda1.jpg";
    }
})

/*******СНЕГ**********/
/*      https://prognote.ru/web-dev/front-end/snow-on-javascript/    */
